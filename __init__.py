...
def iter_circular_prev_this(items):
    prevs, this = tee(items)
...
def add_object(self, context):
    ...
    faces = []
    for prev, this in iter_circular_prev_this([i for i in range(numVerts)]):
    ...
    faces = bpypolyskel.polygonize(
        verts, firstVertIndex, numVerts, holesInfo, height, tan, faces, unitVectors
    )
...
def add_object_button(self, context):
    self.layout.operator(
        OBJECT_OT_add_object.bl_idname, text="Add bpypolykel Demo Object", icon='PLUGIN'
    )
